...
def obs2response(model_instance, agent, obs):
    logits = model_instance({'is_training': False, 'obs': {'observation': torch.from_numpy(np.expand_dims(obs['observation'], 0)), 'action_mask': torch.from_numpy(np.expand_dims(obs['action_mask'], 0))}})
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if t[0] == '0':
            ...
            response = obs2response(model_instance, agent, obs)
            t = response.split()
            if t[0] == 'Hu':
                print('HU')
            elif t[0] == 'Play':
            ...
            if t[2] == 'DRAW':
                ...
                if p == seatWind:
                    ...
                    if t[0] == 'Hu':
                        ...
                        response2 = obs2response(model_instance, agent, obs)
